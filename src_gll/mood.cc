...
// to zero. Previous stage solution is in predictor and we reduce degree of predictor.
//--------------------------------------------------------------------------------------------
template <int dim>
...
bool ConservationLaw<dim>::test_u2(const typename DoFHandler<dim>::cell_iterator &cell)
{
   ...
   for(unsigned int d=0; d<dim; ++d)
   {
      ...
      double d_min = std::min( std::fabs(D2_min[d]), std::fabs(D2_max[d]) );
      double d_max = std::max( std::fabs(D2_min[d]), std::fabs(D2_max[d]) );
      if(d_min < d_max * fact) return false;
   }
...
//         double e = 0.1*(max_mood_var[c] - min_mood_var[c]) + 1.0e-6;
//         double min_val = min_mood_var[c] - e;
//         double max_val = max_mood_var[c] + e;
//         const bool dmp = (mood_var > min_val && mood_var < max_val);
         ...
         else
         {
            ...
            else if(cell_degree[c] == 1 && shock_indicator[c] < 1.0)
            {
               ...
               re_update_new[c] = false;    // aposteriori limiter, no need to reupdate
            }
            else
            {
               unsigned int c = 0;
