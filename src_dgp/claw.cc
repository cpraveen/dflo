...
ConservationLaw<dim>::ConservationLaw (const char *input_filename,
   ...
   :
   fe (FE_DGP<dim>(degree), EulerEquations<dim>::n_components),
   dof_handler (triangulation),
   fe0 (FE_DGP<dim>(0), EulerEquations<dim>::n_components),
   dof_handler0 (triangulation),
   fe_cell (FE_DGP<dim>(0)),
...
void ConservationLaw<dim>::run ()
{
   ...
   */
   ...
   VectorTools::create_right_hand_side (dof_handler,
                                        ...
                                        current_solution);
   inv_mass_matrix.vmult (old_solution, current_solution);
   ...
   while (elapsed_time < parameters.final_time)
   {
      ...
      while(true)
      {
         ...
         if(parameters.solver == Parameters::Solver::rk3)
         {
            // we need current_solution = ark*old_solution + (1-ark)*current_solution
